...
/* Identifiers */
#define NAME         "pkhosts"
#define BRIEF        "Tell and display the hosts cache as viewed on network interface(s)"
#define SYNOPSIS     "pkhosts [options]"
#define DESCRIPTION  "No description yet"

/* Public variable */
pksh_cmd_t cmd_hosts = { NAME, BRIEF, SYNOPSIS, DESCRIPTION, pksh_pkhosts };
...
{
  printf ("`%s' provides a dynamic real-time view of the hosts seen on a given network interface\n", progname);
  ...
  printf ("\n");
  printf ("Examples:\n");
  printf ("   %s -i eth1                     # display the table of hosts viewed on network interface eth1 using default sorting\n", progname);
...
/* Show detailed information about the hosts and their attributes on a given network interface */
int pksh_pkhosts (int argc, char * argv [])
{
  ...
  sf * howtosort = sort_by_ip;        /* default sort by IP address                 */
  int reverse = 0;
  int hostno = 0;
  ...
  while ((option = getopt_long (argc, argv, sopts, lopts, NULL)) != -1)
	...
	case 129:
	  headargv = argsmore (headargv, "--label=IP Address[15]");
	  rowargv = argsmore (rowargv, "--ip-address");
	  break;
	...
	case 131:
	  headargv = argsmore (headargv, "--label=Domain[20]");
	  rowargv = argsmore (rowargv, "--domain=20");
	  break;
	...
	case 134:
	  headargv = argsmore (headargv, "--label=Age[19]");
	  rowargv = argsmore (rowargv, "--age-uptime");
	  break;

	case 135:
	  headargv = argsmore (headargv, "--label=Age[42]");
  ...
  if (optind < argc)
      ...
      for (srchosts = host = hostsall (interface); host && * host; host ++)
	{
	  ...
	  /* Do not include id-less hosts (damn threads!) */
