/*
 * pkwho.c - Tell and display the host cache like the 'who' and 'rwho' commands do for users
 *
 * -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
 *                    _        _
 *              _ __ | | _____| |__
...
#define OPTSTRING "hi:nurlfpPdDx"
  ...
  while ((option = getopt_long (argc, argv, OPTSTRING, long_options, NULL)) != -1)
	...
	case 130:
	  headargv = argsadd (headargv, "--label=System[15]");
	  rowargv = argsadd (rowargv, "--os-name=15");
	  break;
