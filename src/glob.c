...
/* <Rocco Was Here> */
#undef _LIBC
#define __closedir(dir) closedir (dir)
#define __opendir(name) opendir (name)
#define __readdir(str) readdir (str)
#if !defined(__strdup)
...
/* I included here glob.h from glibc-2.3.1 for portability reasons */
...
static void myglobfree (glob_t *pglob);
...
static int
myglob (pattern, flags, errfunc, pglob)
     const char *pattern;
     int flags;
     ...
     glob_t *pglob;
{
  ...
  if (!(flags & GLOB_DOOFFS))
    /* Have to do this so `myglobfree' knows where to start freeing.  It
	  ...
	  if (next == NULL)
	    {
	      ...
	      free (onealt);
#endif
	      return myglob (pattern, flags & ~GLOB_BRACE, errfunc, pglob);
	    }
  ...
  if (filename == NULL)
	...
	{
	  int val = myglob (dirname, flags | GLOB_MARK, errfunc, pglob);
	  if (val == 0)
...
/* Where the myreaddir() has to look for values */
static int gl_argc = 0;
static char ** gl_argv = NULL;

static void * myopendir (__const char * d)
{
  return (void *) 1;   /* True */
}


static void myclosedir (void * p)
{
  ;
}


static int mylstat (const char * file_name, struct stat * buf)
{
  return -1;
}


static int mystat (const char * file_name, struct stat * buf)
{
  return -1;
}


static struct dirent * myreaddir (void * p)
{
  static int i = 0;
  static struct dirent entry;

  entry . d_ino = 59;  /* fake value required by some implementation */

  if (i < gl_argc)
    {
      strcpy (entry . d_name, gl_argv [i ++]);
      return (void *) & entry;
    }
  else
    {
...
char ** globargs (int argc, char * argv [], const char * pattern)
{
  char ** globargv = NULL;
  glob_t globv;
  int gflags = GLOB_ALTDIRFUNC;
  int i;

  if (! argc || ! argv || ! pattern)
    return NULL;
  ...
  globv . gl_pathv    = NULL;
  globv . gl_offs     = 0;
  globv . gl_flags    = gflags;

  globv . gl_opendir  = myopendir;
  globv . gl_readdir  = myreaddir;
  globv . gl_closedir = myclosedir;
  globv . gl_lstat    = mylstat;
  globv . gl_stat     = mystat;

  switch (myglob (pattern, gflags, NULL, & globv))
    {
    case GLOB_ABORTED:       /* aborted */
    case GLOB_NOSPACE:       /* no space left */
      myglobfree (& globv);
      return NULL;

    case GLOB_NOMATCH:       /* no match */
      myglobfree (& globv);
      return NULL;
