...
/* Identifiers */
#define NAME         "packets"
#define BRIEF        "Display detailed information about packets viewed on network interface(s)"
#define SYNOPSIS     "packets [options]"
#define DESCRIPTION  "No description yet"

/* Public variable */
pksh_cmd_t cmd_packets = { NAME, BRIEF, SYNOPSIS, DESCRIPTION, pksh_packets };
...
{
  printf ("`%s' provides a dynamic real-time view of the amount of packets in and out on a given network interface\n", progname);
  ...
  printf ("\n");
  printf ("Examples:\n");
  printf ("   %s -i eth1                      # display the amount of packets viewed on network interface eth1 using default sorting\n", progname);
...
/* Show detailed information about the current traffic (in terms of packets) on a given network interface */
int pksh_packets (int argc, char * argv [])
{
  ...
  while ((option = getopt_long (argc, argv, sopts, lopts, NULL)) != -1)
	...
	case 128:
	  ...
	  rowargv = argsmore (rowargv, "--total-pkts-all");
	  break;

	case 129:
	  headargv = argsmore (headargv, "--label=Tot Sent[9]");
	  rowargv = argsmore (rowargv, "--total-pkts-sent");
	  break;

	case 130:
	  headargv = argsmore (headargv, "--label=Tot Recv[9]");
	  rowargv = argsmore (rowargv, "--total-pkts-recv");
	  break;

	case 131:
	  headargv = argsmore (headargv, "--label=Broadcast[9]");
	  rowargv = argsmore (rowargv, "--broadcast-pkts-sent");
	  break;

	case 132:
	  headargv = argsmore (headargv, "--label=Multicast[9]");
	  rowargv = argsmore (rowargv, "--multicast-pkts-sent");
	  break;

	case 133:
	  headargv = argsmore (headargv, "--label=IP Broad [9]");
	  rowargv = argsmore (rowargv, "--ip-broadcast-pkts-sent");
	  break;

	case 134:
	  headargv = argsmore (headargv, "--label=IP Multi [9]");
	  rowargv = argsmore (rowargv, "--ip-multicast-pkts-sent");
	  break;
