/*
 * bytes.c - Display detailed information about bytes viewed on network interface(s)
 *
 * -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
 *
 *                    _        _
...
{
  printf ("`%s' provides a dynamic real-time view of the amount of bytes in and out on a given interface\n", cmd);
  ...
  printf ("\n");
  printf ("Examples:\n");
  printf ("   %s -i eth1                      # display the amount of bytes viewed on interface eth1 using default sorting\n", cmd);
  ...
  printf ("  --i0, --include-total-all             include column with total bytes (all protocols)\n");
  printf ("  --i1, --include-total-sent            include column with total bytes sent\n");
  printf ("  --i2, --include-total-recv            include column with total bytes received\n");
  printf ("  --i3, --include-broadcast             include column with # data-link broadcast bytes sent\n");
  printf ("  --i4, --include-multicast             include column with # data-link multicast bytes sent\n");
  printf ("  --i5, --include-ip-broadcast          include column with # IP broadcast bytes sent\n");
  printf ("  --i6, --include-ip-multicast          include column with # IP multicast bytes sent\n");
  ...
  printf ("  --s6, --sort-by-broadcast-bytes       sort the hosts cache by total # of data-link broadcast bytes sent\n");
  printf ("  --s7, --sort-by-multicast-bytes       sort the hosts cache by total # of data-link multicast bytes sent\n");
  printf ("  --s8, --sort-by-ip-broadcast-bytes    sort the hosts cache by total # of IP broadcast bytes sent\n");
  printf ("  --s9, --sort-by-ip-multicast-bytes    sort the hosts cache by total # of IP multicast bytes sent\n");
}


/* Show detailed information about the current traffic (in terms of bytes) on a given interface */
int pksh_bytes (int argc, char * argv [])
{
  ...
  static struct option const long_options [] =
    {
      ...
      { "include-broadcast",            no_argument,       NULL, 131 },
      { "include-multicast",            no_argument,       NULL, 132 },
      { "include-ip-broadcast",         no_argument,       NULL, 133 },
      { "include-ip-multicast",         no_argument,       NULL, 134 },
      ...
      { "sort-by-broadcast-bytes",      no_argument,       NULL, 234 },
      { "sort-by-multicast-bytes",      no_argument,       NULL, 235 },
      { "sort-by-ip-broadcast-bytes",   no_argument,       NULL, 236 },
      { "sort-by-ip-multicast-bytes",   no_argument,       NULL, 237 },
  ...
  /*  Host Id |  Total  |Tot Sent |Tot Recv |Broadcast|Multicast| */
  char * head [] =
    { argv [0],
...
#define OPTSTRING "hi:nurlfpPdDx"
  ...
  while ((option = getopt_long (argc, argv, OPTSTRING, long_options, NULL)) != -1)
	...
	case 129:
	  ...
	  rowargv = argsadd (rowargv, "--total-bytes-sent");
	  break;

	case 130:
	  headargv = argsadd (headargv, "--label=Tot Recv[9]");
	  rowargv = argsadd (rowargv, "--total-bytes-recv");
	  break;

	case 131:
	  headargv = argsadd (headargv, "--label=Broadcast[9]");
	  rowargv = argsadd (rowargv, "--broadcast-bytes");
	  break;
	...
	case 133:
	  ...
	  rowargv = argsadd (rowargv, "--broadcast-ip-bytes");
	  break;

	case 134:
	  headargv = argsadd (headargv, "--label=IP Multi [9]");
	  rowargv = argsadd (rowargv, "--multicast-ip-bytes");
	  break;
